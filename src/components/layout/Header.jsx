...
// Icône hamburger personnalisée (barres en escalier)
const HamburgerIcon = () => (
  <svg
    width="20"
    height="16"
...
// ─── LIENS DE NAVIGATION (clés de traduction) ────────────────────
const NAV_LINKS = [
  { key: 'explore',     href: '/' },
...
const ECOSYSTEM_ITEMS = [
  ...
  [
    { key: 'health', href: '/secteurs/sante', icon: <HealthIcon style={{ width: 14 }} /> },
...
const STYLES = `
  ...
  .ph-overlay {
    ...
    z-index: 300;
    animation: ph-fade-in 0.22s ease forwards;
  }
  ...
  .ph-drawer {
    ...
    display: flex;
    flex-direction: column;
    animation: ph-slide-in 0.25s ease forwards;
    overflow-y: auto;
  }
  ...
  /* BURGER SANS BORDURE + ÉCHELLE (icône personnalisée) */
  .ph-burger {
    display: none;
    align-items: center;
...
const useTheme = () => {
  ...
  useEffect(() => {
    ...
    Object.entries(colors).forEach(([key, val]) => {
      document.documentElement.style.setProperty(`--hdr-${key.replace(/([A-Z])/g, '-$1').toLowerCase()}`, val);
    });
  }, [theme]);
        ...
        <div ref={dropdownRef} className="ph-resources-menu">
          <div className="ph-resources-section">
            ...
            <div className="ph-lang-options">
              <button className={`ph-lang-opt ${currentLang === 'fr' ? 'active' : ''}`} onClick={() => changeLanguage('fr')}>Français</button>
              <button className={`ph-lang-opt ${currentLang === 'ht' ? 'active' : ''}`} onClick={() => changeLanguage('ht')}>Kreyòl</button>
            </div>
          </div>
          <div className="ph-resources-section">
            <div className="ph-resources-label">{t('appearance')}</div>
            <div className="ph-darkmode-toggle">
              <span style={{ fontSize: 13, fontFamily: "var(--hdr-font)" }}>{theme === 'light' ? t('light_mode') : t('dark_mode')}</span>
              <div className={`ph-toggle-switch ${theme === 'dark' ? 'dark' : ''}`} onClick={darkModeToggle}>
...
const MobileDrawer = ({ isOpen, onClose, isActive, searchQuery, setSearchQuery, darkModeToggle, theme }) => {
  ...
  useEffect(() => {
    const themeColor = theme === 'dark' ? '#0B0E14' : '#FFFFFF';
      ...
      <div className={`ph-drawer${closing ? ' closing' : ''}`} role="dialog" aria-modal="true">
        ...
        <div className="ph-drawer-search">
          <div style={{ position: 'relative' }}>
            <MagnifyingGlassIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 16, color: 'var(--hdr-text-muted)' }} />
            <input
              type="text"
        ...
        <nav className="ph-drawer-nav">
          {NAV_LINKS.map(link => (
            <Link key={link.href} to={link.href} onClick={handleClose} className={`ph-drawer-link${isActive(link.href) ? ' is-active' : ''}`}>
              {t(link.key)}
            </Link>
          ))}
        </nav>
        ...
        <div className="ph-drawer-ecosystem">
          <div className="ph-drawer-res-title" style={{ color: 'var(--hdr-accent)' }}>{t('ecosystems').toUpperCase()}</div>
          {ECOSYSTEM_ITEMS.map((column, idx) => (
            <div key={idx}>
              {column.map(item => (
        ...
        <div className="ph-drawer-resources">
          ...
          <div className="ph-drawer-lang-options">
            <button className={`ph-drawer-lang-btn ${currentLang === 'fr' ? 'active' : ''}`} onClick={() => changeLanguage('fr')}>Français</button>
            <button className={`ph-drawer-lang-btn ${currentLang === 'ht' ? 'active' : ''}`} onClick={() => changeLanguage('ht')}>Kreyòl</button>
          </div>
        ...
        <div className="ph-drawer-footer">
          <div className="ph-drawer-cta-row">
            <Link to="/connexion" onClick={handleClose} className="ph-drawer-btn-outline">{t('login')}</Link>
            <Link to="/inscription" onClick={handleClose} className="ph-drawer-btn-light">{t('signup')}</Link>
          </div>
        </div>
      </div>
    </>
      ...
      >
        <div style={{
          ...
          <div className="ph-search-wrapper">
            {/* Optionnel : réactiver la recherche desktop */}
          </div>

          {/* Navigation desktop */}
          <nav className="ph-desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1, justifyContent: 'center' }}>
            {NAV_LINKS.map(link => (
              <Link key={link.href} to={link.href} className={`ph-nav-link${isActive(link.href) ? ' is-active' : ''}`}>
                {t(link.key)}
              </Link>
            ))}
            <EcosystemMegaMenu />
          ...
          {/* Actions desktop */}
          <div className="ph-desktop-actions" style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
